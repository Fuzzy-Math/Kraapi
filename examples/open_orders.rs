...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let open_orders = KIOpenOrders::build().with_trade_info(true).finish();
    ...
    let orders = vec![
        String::from("OSZ3EZ-UIDEH-LRTIXW"),
        String::from("OZIDYB-CLNS5-PGGLZG"),
    ];
    let queried_orders = KIOrderInfo::build_with_list(orders).finish();
